...
<Canvas>
  <Story
    name="disabled"
    args={{disabled: true, alwaysDown: true, maxItems: 4, data: people, placeholder: "Select person", label: (item) => item.name, onSearch: null}}>
    {NoFormTemplate.bind({})}
  </Story>
</Canvas>
...
<Canvas>
  <Story
    name="error"
    args={{error: true, alwaysDown: true, maxItems: 4, data: people, placeholder: "Select person", label: (item) => item.name, onSearch: null}}>
    {NoFormTemplate.bind({})}
  </Story>
</Canvas>
